...
def get_required_kw_args(fn):
    args = []
    # inspect.signature(fn)是返回fn函数中所有的参数
    # inspect.signature(fn).parameters返回的是一个映射（mappingproxy）类型的对象，值为一个有序字典
    # 这个字典里的key是即为参数名，str类型，value是一个inspect.Parameter类型的对象
...
# 封装一个url处理函数，目的是从url函数中分析其需要接受的参数
